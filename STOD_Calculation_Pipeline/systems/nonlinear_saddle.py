# ----------------------------------------------------------------------
#        SYSTEM DEFINITION: Nonlinear Hyperbolic Saddle (2D)
# ----------------------------------------------------------------------
# Contract for the generic pipeline:
...
def get_velocity_field(state, t, physics_params):
    """
    Nonlinear saddle with curved invariant manifolds:
        dx/dt =  +alpha * x + beta * x * y
        dy/dt =  -alpha * y + gamma * x**2
    ...
    state : array_like, shape (2,)
        [x, y]
    t : float
        time (unused but kept for integrator API compatibility)
    physics_params : dict
...
def discretize_trajectory_to_grid(trajectory, grid_params):
    ...
    Keeps duplicates (one sample → one cell) to match SOD semantics.
    """
    nx = int(grid_params['grid_resolution_x'])
    ny = int(grid_params['grid_resolution_y'])
    x_min, x_max = float(grid_params['x_min']), float(grid_params['x_max'])
